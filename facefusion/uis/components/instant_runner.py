...
from facefusion.jobs import job_manager, job_helper, job_runner, job_store
...
def render() -> None:
    global INSTANT_RUNNER_START_BUTTON, INSTANT_RUNNER_STOP_BUTTON, INSTANT_RUNNER_CLEAR_BUTTON
...
def remote_update(ui_workflow: str) -> gradio.Row:
...
def create_and_run_job(step_args: dict) -> bool:
...
def start_gradio_ui():
    port = int(os.environ.get("PORT", 8080))  # Cloud Run uses PORT env var, default to 8080
    iface = gr.Interface(fn=run, inputs=..., outputs=...)  # Define inputs and outputs as per your app's needs
