...
def open_curl(args: List[str]) -> subprocess.Popen[bytes]:
    # Prepare curl command and return process
    ...
    for url in urls:
        ...
        if initial_size < download_size:
            with tqdm(total=download_size, initial=initial_size, desc=wording.get('downloading'), unit='B', unit_scale=True, unit_divisor=1024, ascii=' =', disable=state_manager.get_item('log_level') in ['warn', 'error']) as progress:
                commands = ['--create-dirs', '--continue-at', str(initial_size), '--output', download_file_path, url]
                process = open_curl(commands)

                current_size = initial_size
                progress.set_postfix(download_providers=[], file_name=download_file_name)
                ...
                process.communicate()  # Ensure that the process is fully completed
...
def resolve_download_url(base_name: str, file_name: str) -> Optional[str]:
    download_providers = []  # This is set to empty to skip downloads

    if not download_providers:
        logger.info("No download providers available, skipping download.", module_name=__name__)
        return None  # No download URL is resolved, return None

    # The following code won't run because the condition above will always be true with empty providers
    ...
    logger.info("No valid download providers found after checking.", module_name=__name__)
    return None
