...
def download_model_from_gcs(bucket_name: str, cloud_model_path: str, local_model_path: str) -> None:
    """Download the model from GCS to the .assets directory."""
    storage_client = create_storage_client()
    bucket = storage_client.bucket(bucket_name)
    blob = bucket.blob(cloud_model_path)

    # Ensure the local directories exist in the .assets folder
    os.makedirs(os.path.dirname(local_model_path), exist_ok=True)

    # Download the model to .assets in Cloud Run (or any subdirectory within it)
...
def get_static_model_initializer(model_path: str) -> ModelInitializer:
    ...
    if not os.path.exists(model_path):
        ...
        # Ensure the model gets downloaded to .assets/models/{filename} in Cloud Run
        download_model_from_gcs(BUCKET_NAME, cloud_model_path, f".assets/models/{filename}")
        
        model_path = f".assets/models/{filename}"  # Set the model path to .assets/models/{filename} in Cloud Run
